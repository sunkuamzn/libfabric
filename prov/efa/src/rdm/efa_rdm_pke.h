 ...
 * Target size: 512 bytes total (balance between debug history and memory overhead)
 * - 48 entries provide sufficient history for diagnosis
 * - Keeps struct at 4x cache lines (512 bytes) vs 8x (1024 bytes)
 */

/* Calculate maximum debug entries that fit in available space */
#define EFA_RDM_PKE_DEBUG_INFO_SIZE 48  /* (512 - 120 base - 1 counter) / 8 bytes per entry */
...
struct efa_rdm_pke_debug_info {
	...
	uint8_t event;        /**< Event type (4 bits used) */
};
...
#if ENABLE_DEBUG
	struct efa_rdm_pke_debug_info debug_info_vec[EFA_RDM_PKE_DEBUG_INFO_SIZE]; /**< Circular buffer of debug events */
	uint8_t debug_info_counter; /**< Incremental counter (0-255) */
#endif
...
{
	uint8_t idx = pkt_entry->debug_info_counter % EFA_RDM_PKE_DEBUG_INFO_SIZE;
	struct efa_rdm_pke_debug_info *info = &pkt_entry->debug_info_vec[idx];

	info->qkey = qkey;
	EFA_RDM_PKE_DEBUG_INFO_SET_QPN_GEN(info, qpn, gen);
	info->counter = pkt_entry->debug_info_counter;
	info->event = event_type;
	pkt_entry->debug_info_counter++;
}
