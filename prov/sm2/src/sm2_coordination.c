...
ssize_t sm2_mmap_unmap_and_close(struct sm2_mmap *map )
{
	...
	err1 = close(map->fd);
	if (err1) {
		FI_WARN(&sm2_prov, FI_LOG_AV, "Failed to close fd %d with error code %d", map->fd, err1);
		first_failed = true;
	}

	err2 = munmap(map->base, map->size);
	if (err2) {
		FI_WARN(&sm2_prov, FI_LOG_AV, "Failed unmap shared memory region with error code %d", err2);
	}
